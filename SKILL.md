...
description: |
  ...
  AI 词汇、否定式排比、过多的连接性短语，以及论文中过度防御、自我否定的写法。
allowed-tools:
  - Read
  - Write
  - Edit
...
学术文本需要边界，但边界说明不应写成免责声明。很多 AI 润色会把论文改成“我们没有证明什么、不能说明什么、只是做了什么”的防守姿态，读者会先感到研究很弱，而不是看到贡献在哪里。

### 需要警惕的信号

- 一段话连续出现“不是、不能、不构成、不保证、仅、只是、边界、局限、退化、妥协、保守、无法、未完成”。
- 先讲限制，再讲方法价值。
- 把每个理论边界都写成“本文不……”。
- 用“免责声明”替代机制解释，例如“这不是严格证明”后面没有说明本文到底采用了什么可复核机制。
- 结果段反复强调失败、退化和不能外推，却没有先交代正结果、机制和适用范围。

### 改写原则

1. **先贡献，后边界。** 每段先说方法解决了什么、为什么这样设计、带来什么可验证结果；最后一句再给适用范围。
2. **把否定句改成机制句。** 不写“本文没有做 X”，优先写“本文采用 Y 处理 X 的风险”。
3. **把妥协写成设计选择。** 如果是工程预算、验证成本或理论条件导致的选择，说明这个选择如何保证可复核性、可比较性或搜索效率。
4. **限制要具体，不要自毁。** 写清楚限制发生在哪个条件、哪个算例、哪个协议下；不要泛化成“方法不可靠”。
5. **少用“只是/仅/不构成”。** 必须保留边界时，尽量用“适用于……”“本文将其定位为……”“结果支持……范围内的结论”。
6. **边界说明放在段尾。** 不让读者第一眼看到防守，而是先看到研究主线。

### 常见改法

| 过度防御写法 | 更好的写法 |
|------|------|
| 本文不构造全局有效的解析排放灵敏度。 | 本文采用“场景排放聚合反馈 + 完整固定点验证”处理用户年度合规，避免把单点排放结果外推到任意投资点。 |
| 该方法不保证全局最优。 | 该方法把学习模型限制在候选筛选环节，最终成本排序由完整物理验证给出。 |
| 这只是一个补充实验。 | 该实验用于回答扩维离散空间中的搜索压缩问题，和主线六维规划结果形成互补。 |
| 该结果不能外推到所有场景。 | 该结果支持训练分布附近的候选生成有效性；强扰动场景另行报告稳健性边界。 |
| 本文没有完成严格统计充分性证明。 | 本文按统一场景协议比较各方法，并通过场景审计说明样本覆盖特征。 |

### 论文段落检查

- 是否每段都先讲“做不到什么”？
- 是否把工程选择写成了“妥协”而不是“可复核设计”？
- 是否用边界说明压过了正结果？
- 是否把局部负结果写成了整体失败？
- 是否有连续三句以上是否定式说明？如果有，重排顺序：贡献 -> 机制 -> 证据 -> 范围。
...
**问题：** GPT 在中英互译或润色时，容易把多个英文词统一译成"口径"，让中文显得像机器翻译或公文套话。处理时先判断英文词的真实含义，再决定保留"口径"还是改成更自然的中文。

**常见英文对应关系：**

| English term/phrase | GPT 可能译成 | 更自然的中文 |
|------|------|------|
| definition | 定义口径 | 定义、界定方式 |
| scope | 统计口径、覆盖口径 | 范围、覆盖范围 |
| criteria | 判断口径、认定口径 | 标准、判断标准 |
| methodology | 测算口径、计算口径 | 方法、计算方法 |
| basis | 披露口径、核算口径 | 基础、依据、标准 |
| reporting basis | 报告口径、披露口径 | 报告方式、披露标准 |
| comparable basis | 可比口径 | 可比标准、同一标准下 |
| framework | 分析口径 | 分析框架、分析角度 |
| framing | 表述口径 | 说法、表达方式 |
| narrative | 叙事口径 | 叙事方式、说法 |
| interpretation / understanding | 理解口径 | 理解方式、理解角度 |
| discussion scope | 讨论口径 | 讨论范围、讨论前提 |
...
**处理原则：** basis 是最常见来源。只有在统计、财务、监管、报告披露等语境中确实强调"同一计算/披露标准"时，才保留"口径"；否则优先改成"标准""范围""方法""依据""说法"等更具体的词。

**改写前：**
> 本报告采用统一披露口径，对各业务线收入进行比较。
...
**任务：** 在不改变原意、不删减关键信息的前提下，把明显带有 AI 味、公文味、咨询报告味、机关腔、过度抽象化的表达，改成更自然、清楚、像真人会说的现代中文。重点处理"口径"一类滥用词：它们可能来自翻译误差，也可能来自 AI 惰性套话。

**处理原则：**
- 先判断语境，再替换；不要为了替换而替换。
- 保留原文逻辑结构和信息重点。
- 口语和书面语之间保持平衡，不要把正式文本改得过于随意。
- 技术、组织或产品语境中确实作为专有名词使用的词可以保留，但要删掉空泛堆叠。

**常见替换方向：**

| 抽象词 | 更自然的表达 |
|------|------|
| 口径 | 标准、范围、说法、定义、计算方式、统计方式、理解方式、角度 |
| 维度 | 方面、角度、层面、指标 |
| 场景 | 情况、使用情况、应用情况、具体情况 |
| 路径 | 方法、方式、步骤、办法 |
| 抓手 | 关键办法、切入点、着力点 |
| 赋能 | 帮助、支持、提升、让……更容易 |
| 闭环 | 完整流程、把事情做完、有反馈、有结果 |
| 对齐 | 统一、说清楚、达成一致、保持一致 |
| 落地 | 实施、执行、真正做起来、变成实际行动 |
| 沉淀 | 积累、总结、留下、形成 |
| 颗粒度 | 详细程度、细致程度 |
| 底层逻辑 | 根本原因、基本原理、核心思路 |
| 方法论 | 方法、经验、做法、思路 |
...
**改写前：**
> 我们需要从用户心智和业务场景两个维度出发，拉齐项目口径，打通全链路流程，形成可持续的长效机制。

**改写后：**
> 我们需要先说清楚用户怎么看、业务里会怎么用，再统一项目标准，理顺整个流程，形成长期有效的办法。

---

### 补充：论文写作中避免程序变量名和随意缩写

**问题：** 论文文本不能写得像代码注释、实验报告或内部方案。不要直接使用程序变量名、中英文混合词、纯英文组合词、自定义缩写或临时缩写，例如 `exact_uc`、`AACJKH`、`xxx_method`。这些表达会让论文显得不正式，也会让读者无法判断概念含义。

**处理原则：**
- 将程序变量名改成合理的中文术语，必要时保留括号中的英文全称或缩写。
- 常见领域缩写可以保留，但首次出现时必须给出中文名称和定义。
- 作者提出的方法、模型、指标或主题首次出现时，必须先定义，再使用简称。
- 不常见、自造或无法解释的缩写不要直接保留；改成中文描述，或要求作者补充定义。
- 变量名、文件名、字段名只应出现在代码、表格字段说明或附录中；正文叙述中应改成自然中文。

**常见处理方式：**

| 不推荐写法 | 更适合论文正文的写法 |
|------|------|
| exact_uc 方法 | 精确不确定约束处理方法 |
| AACJKH 指标 | 该指标；若必须使用，首次出现时定义 AACJKH 的完整含义 |
| custom_strategy | 自定义策略 |
| user_level_feature | 用户层面特征 |
| loss_weight | 损失权重 |
| GBD | 广义 Benders 分解（Generalized Benders Decomposition，GBD） |

**改写前：**
> 本文提出 exact_uc 方法，并将其与 GBD 结合，用于提升 AACJKH 指标表现。

**改写后：**
> 本文提出一种精确不确定约束处理方法，并将其与广义 Benders 分解（Generalized Benders Decomposition，GBD）结合，用于提升目标评价指标的表现。若 AACJKH 是本文自定义指标，应在首次出现时说明其完整名称、计算方式和含义。
